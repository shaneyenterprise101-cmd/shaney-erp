...
export default function Envelope() {
    ...
    const fetchEnvelopeDesign = async () => {
      try {
        ...
        if (res.ok) {
          ...
          } else if (allData.envelope_designs && Array.isArray(allData.envelope_designs)) {
            cloudDesigns = allData.envelope_designs.filter(item => String(item.firmId) === String(selectedFirm));
          }
  ...
  const saveDesign = async () => {
    ...
    try {
      // Save to AWS DynamoDB & SQLite IPC
      await fetch(`${BACKEND_URL}/api/data`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'envelope_designs', id: String(selectedFirm), data: data })
      });
      if (window.require) {
