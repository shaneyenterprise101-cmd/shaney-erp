...
export default function Envelope() {
  ...
  const [toMobile, setToMobile] = useState(''); // 🟢 Mobile Number State
  ...
  // Firm-wise design load effect with local background preservation
  useEffect(() => {
    if (!selectedFirm) return;
    
    const fetchAllEnvelopeDesigns = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (allData && typeof allData === 'object') {
            ...
            if (Array.isArray(allData)) {
              cloudDesigns = allData.filter(item => item.docType === 'envelope_design' && String(item.firmId) === String(selectedFirm));
            } else if (allData.settings) {
              const allSettings = Array.isArray(allData.settings) ? allData.settings : Object.values(allData.settings);
              cloudDesigns = allSettings.filter(item => item && item.docType === 'envelope_design' && String(item.firmId) === String(selectedFirm));
            }
        ...
        if (activeDesign) {
          setSizeSelect(activeDesign.sizeSelect || 'DL-PORT');
          setCustomWidth(activeDesign.customWidth || 109);
          setCustomHeight(activeDesign.customHeight || 239);
          setMarginTop(activeDesign.marginTop || 0);
          setMarginBottom(activeDesign.marginBottom || 0);
          setMarginLeft(activeDesign.marginLeft || 0);
          setMarginRight(activeDesign.marginRight || 0);
          setFontFamily(activeDesign.fontFamily || 'Arial');
          setFontSize(activeDesign.fontSize || 18);
          setIsBold(activeDesign.isBold || false);
          setIsItalic(activeDesign.isItalic || false);
          setIsUnderline(activeDesign.isUnderline || false);
          setTextColor(activeDesign.textColor || '#000000');
          setTextRotate(activeDesign.textRotate || '0');
          setTextX(activeDesign.textX || 30);
          setTextY(activeDesign.textY || 120);
          setCustomText(activeDesign.customText || '');
          setCustomTextFont(activeDesign.customTextFont || 'Arial');
          setCustomTextSize(activeDesign.customTextSize || 14);
          setCustomTextBold(activeDesign.customTextBold || false);
          setCustomTextItalic(activeDesign.customTextItalic || false);
          setCustomTextUnderline(activeDesign.customTextUnderline || false);
          setCustomTextColor(activeDesign.customTextColor || '#000000');
          setCustomTextW(activeDesign.customTextW || 200);
          setCustomTextR(activeDesign.customTextR || 0);
          setCustomTextX(activeDesign.customTextX || 10);
          setCustomTextY(activeDesign.customTextY || 10);
          setEnvBg(activeDesign.envBg || (localDesign ? localDesign.envBg : null));
          setBgW(activeDesign.bgW || 100);
          setBgH(activeDesign.bgH || 100);
          setBgX(activeDesign.bgX || 0);
          setBgY(activeDesign.bgY || 0);
          setImg1(activeDesign.img1 || (localDesign ? localDesign.img1 : null));
          setImg1W(activeDesign.img1W || 50);
          setImg1X(activeDesign.img1X || 10);
          setImg1Y(activeDesign.img1Y || 50);
          setImg2(activeDesign.img2 || (localDesign ? localDesign.img2 : null));
          setImg2W(activeDesign.img2W || 50);
          setImg2X(activeDesign.img2X || 10);
          setImg2Y(activeDesign.img2Y || 100);
        } else {
          setEnvBg(null);
          setCustomText('');
          setImg1(null);
        ...
        if (localDesign) {
          setEnvBg(localDesign.envBg || null);
          setImg1(localDesign.img1 || null);
          setImg2(localDesign.img2 || null);
        }
      }
    };

    fetchAllEnvelopeDesigns();
  }, [selectedFirm]);

  // 🟢 OLD & RELIABLE CUSTOMER SELECT METHOD
  const handleCustomerSelect = (nameVal) => {
    ...
    if (found) {
      ...
      const primaryMobile = found.contacts && found.contacts.length > 0 ? found.contacts[0].mobile : '';
      setToMobile(primaryMobile);
    }
  };
  ...
  const handleBgUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      img.onload = () => {
        ...
        setEnvBg(compressedDataUrl);
      };
    };
    reader.readAsDataURL(file);
  };

  // 🟢 COMPRESSED CUSTOM IMAGES UPLOAD
  const handleImgUpload = (e, num) => {
    ...
    reader.onload = (evt) => {
      ...
      img.onload = () => {
        ...
        const compressedDataUrl = canvas.toDataURL('image/jpeg', 0.8);
        if (num === 1) setImg1(compressedDataUrl);
        else setImg2(compressedDataUrl);
      };
    };
    reader.readAsDataURL(file);
  };

  // 🟢 FIRM-WISE CLOUD SAVE WITH COMPRESSED IMAGES
  const saveDesign = async () => {
    ...
    try {
      ...
      logActionToBackend(`Saved Envelope Design for firm ID: ${selectedFirm}`);
      alert('✅ Envelope design saved and synced to Cloud successfully across all devices!');
    } catch (err) {
      console.error("Cloud envelope save error:", err);
  ...
  const renderEnvelopeContent = () => (
    ...
    >
      {/* Receiver Address Box with Address on Top and Mob Number Below */}
      <div 
        className="absolute"
        style={{
          left: `${textX}mm`,
      ...
      >
        ...
        {/* 🟢 MOBILE NUMBER DIRECTLY BELOW ADDRESS AS Mob:-[Number] */}
        {toMobile && (
          <p 
      ...
      <div className="max-w-7xl mx-auto pb-10">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
          ...
          <div className="w-full lg:w-[420px] shrink-0 bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col gap-4 lg:max-h-[85vh] lg:overflow-y-auto custom-scrollbar">
            ...
            {/* 2. RECEIVER (TO) WITH OLD RELIABLE DATALIST & MOBILE FIELD */}
            <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
              ...
              <input 
                ...
                className="pro-input w-full mb-2 text-xs font-mono font-bold bg-white" 
              />
