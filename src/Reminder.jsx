...
export default function Reminder({ selectedFY }) {
  ...
  const [historyData, setHistoryData] = useState(() => {
    ...
    return saved ? JSON.parse(saved).filter(b => b.docType === 'certificate') : [];
  });
    ...
    const handleDataUpdate = (e) => {
      if (!e.detail || e.detail.type === 'certificates' || e.detail.type === 'customers') {
        try {
          const saved = localStorage.getItem('ERP_History_v104');
          if (saved) {
            setHistoryData(JSON.parse(saved).filter(b => b.docType === 'certificate'));
          }
  ...
  const handleWhatsAppSend = async (e, cert) => {
    ...
    if (idx !== -1) {
      ...
      try {
        await setDoc(doc(db, "certificates", String(cert.id)), sanitizeForFirebase(targetRecord), { merge: true });
        ...
        console.error("Firebase/SQLite whatsapp sent update error:", err);
      }
    }
  ...
  const setNeverReturnStatus = async (e, id, statusVal) => {
    ...
    try {
      await setDoc(doc(db, "certificates", String(id)), sanitizeForFirebase(targetRecord), { merge: true });
      ...
      console.error("Firebase/SQLite neverReturn update error:", err);
    }
  };
  ...
  const toggleSingleReminderStatus = async (e, id, statusVal) => {
    ...
    try {
      ...
      console.error("Firebase/SQLite reminder update error:", err);
    }
  };
  ...
  const markBulkRemindersDone = async () => {
    ...
    for (let rec of updatedRecords) {
      try {
        await setDoc(doc(db, "certificates", String(rec.id)), sanitizeForFirebase(rec), { merge: true });
        ...
        console.error("Firebase/SQLite bulk reminder update error:", err);
      }
    }
      ...
      filteredReminders.forEach((r, index) => {
        setTimeout(async () => {
          ...
          if (idx !== -1) {
            ...
            try {
              await setDoc(doc(db, "certificates", String(r.id)), sanitizeForFirebase(targetRecord), { merge: true });
              ...
              console.error("Firebase/SQLite auto reminder update error:", err);
            }
          }
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
            ...
            <div className="p-3 bg-white border-t border-slate-200 flex justify-end shrink-0">
              <button onClick={() => setIsNeverReturnModalOpen(false)} className="bg-slate-800 hover:bg-black text-white px-6 py-2 rounded-xl font-black text-xs uppercase tracking-widest transition-all cursor-pointer">
                Close
              </button>
            </div>
