...
export default function Quotation({ selectedFY, initialViewMode }) {
    ...
    const handleDataUpdate = (e) => {
      if (!e.detail || e.detail.type === 'quotations') {
        ...
        if (saved) {
          ...
          // 🟢 Migration fallback check for older records missing updatedAt
  ...
  // 🟢 Fetch quotations from Firebase Cloud on load
  useEffect(() => {
    const fetchCloudQuotations = async () => {
      try {
        ...
        const cloudQuotes = cloudHistory.filter(b => b.docType === 'quotation');
  ...
  const handleSaveQuotation = async (e, directAction = 'save') => {
    ...
    try {
      if (quoteObj) {
        await setDoc(doc(db, "history", String(targetQuoteId)), quoteObj);
      ...
      console.error("Firebase quotation save error:", err);
    }
  ...
  const handleDeleteHistory = async (e, id) => {
    ...
    if(confirm('Are you sure you want to delete this quotation?')) {
      ...
      try {
        ...
        console.error("Firebase quotation delete error:", err);
      }
  ...
  const handleWhatsAppSend = async (e, quote) => {
    ...
    if (idx !== -1) {
      ...
      setQuotations(allHistory.filter(b => b.docType === 'quotation'));
      try {
        await setDoc(doc(db, "history", String(quote.id)), sanitizeForFirebase(targetRecord), { merge: true });
  ...
  // 🟢 CLIENT-SIDE CACHING MECHANISM FOR QUOTATION VIEWER (0 Firebase Reads after 1st load)
  const handleViewQuote = async (e, q) => {
    ...
    try {
      ...
      if (cached) {
        ...
        const docRef = doc(db, "history", String(q.id));
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          quoteToView = docSnap.data();
